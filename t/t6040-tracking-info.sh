...
test_expect_success 'status.compareBranches shows ahead of both upstream and push branch' '
	(
		...
		advance work &&
		git -c status.compareBranches="@{upstream} @{push}" status >../actual
	) &&
	cat >expect <<-EOF &&
	On branch feature2
...
test_expect_success 'checkout with status.compareBranches shows both branches' '
	(
		cd test &&
		git -c status.compareBranches="@{upstream} @{push}" checkout feature2 >../actual
	) &&
	cat >expect <<-EOF &&
	Your branch is ahead of ${SQ}origin/main${SQ} by 1 commit.
	  (use "git push" to publish your local commits)
...
test_expect_success 'status --no-ahead-behind with status.compareBranches' '
	(
		cd test &&
		git checkout feature4 &&
		git -c status.compareBranches="@{upstream} @{push}" status --no-ahead-behind >../actual
	) &&
	cat >expect <<-EOF &&
	On branch feature4
	Your branch and ${SQ}origin/main${SQ} refer to different commits.
...
test_expect_success 'checkout with status.compareBranches shows up to date' '
	(
		cd test &&
		git -c status.compareBranches="@{upstream} @{push}" checkout feature7 >../actual
	) &&
	cat >expect <<-EOF &&
	Your branch is up to date with ${SQ}upstream/main${SQ}.
