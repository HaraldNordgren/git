...
test_expect_success '--delete-merged hands its upstream down to a stacked branch' '
	test_when_finished "rm -rf repo" &&
	setup_repo_for_delete_merged &&
	...
	test_grep "Reset upstream of .topic. to .refs/remotes/origin/next." err &&
	test_must_fail git -C repo rev-parse --verify refs/heads/feature &&
	echo refs/remotes/origin/next >expect &&
	git -C repo rev-parse --symbolic-full-name topic@{upstream} >actual &&
	test_cmp expect actual
'

test_expect_success '--delete-merged clears a merged stack in one sweep' '
	test_when_finished "rm -rf repo" &&
	setup_repo_for_delete_merged &&
	(
		cd repo &&
		git branch b1 origin/next &&
		git branch --set-upstream-to=origin/next b1 &&
		git branch b2 origin/next &&
		git branch --set-upstream-to=b1 b2 &&
		git branch b3 origin/next &&
		git branch --set-upstream-to=b2 b3 &&
		git checkout --detach
	) &&
