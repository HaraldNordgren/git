...
+
Combining `--left-only` and `--right-only` suppresses commits on both
sides that have no counterpart, leaving only the commits that
correspond between the two ranges.
