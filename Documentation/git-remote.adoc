...
+
With `--set-head` option, the symbolic-ref `refs/remotes/<name>/HEAD` is set
up to track the remote's default branch, in the same way as `git remote
set-head <name> --auto`. This implies `-f`, since the remote must be fetched
to determine its `HEAD`. Cannot be combined with `-m` or `--mirror`.
+
