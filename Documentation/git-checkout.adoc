...
_<start-point>_ is in _<remote-tracking-namespace>/<branch>_ form, only
that branch is updated; when _<start-point>_ is a bare
_<remote-tracking-namespace>_ (e.g. `origin`), the branch named by
_<remote-tracking-namespace>/HEAD_ is updated, and the checkout fails
if no such symref is configured. If the fetch fails and the
